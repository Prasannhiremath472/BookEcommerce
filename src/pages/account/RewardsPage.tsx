    ...
    <div>
      ...
      >
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-white/70">Reward Points</span>
        </div>
        <p className="mt-4 font-heading text-4xl font-bold">{rewardPoints.toLocaleString()} pts</p>
        <p className="mt-1 text-sm text-white/60">≈ {(rewardPoints / 20).toFixed(2)} in redeemable credit</p>

        <div className="mt-6">
          <div className="mb-2 flex items-center justify-between text-xs text-white/60">
            <span className="flex items-center gap-1"><Sparkles size={12} /> Gold Tier</span>
            <span>{rewardPoints} / {nextTier} pts</span>
          </div>
      ...
      <h2 className="mb-4 font-heading text-lg font-bold text-ink">Points History</h2>
      <div className="flex flex-col divide-y divide-ink/8 rounded-2xl border border-ink/8 bg-white">
        {rewardHistory.map((r) => (
          <div key={r.id} className="flex items-center justify-between p-5">
            ...
            <span className="font-heading text-sm font-bold text-accent-600">+{r.points} pts</span>
          </div>
        ))}
      </div>
    </div>
